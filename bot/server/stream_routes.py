...
from bot.clone import db, cdb # Both
...
async def find_file_in_dbs(unique_id):
    f = await db.files.find_one({"file_unique_id": unique_id})
    if f: return f
    return await cdb.files.find_one({"file_unique_id": unique_id})
...
async def get_file_info(message_id: int):
    try:
        ...
        if not msg or not msg.media: return JSONResponse({"error": "File not found"}, status_code=404)
        media = msg.document or msg.video or msg.audio
        file_name = getattr(media, "file_name", "file.bin")
        file_size = getattr(media, "file_size", 0)
        is_locked = False
        if hasattr(media, "file_unique_id"):
            file_data = await find_file_in_dbs(media.file_unique_id)
            if file_data:
                if file_data.get("password"): is_locked = True
                if file_data.get("file_name"): file_name = file_data["file_name"]
        return {"file_name": file_name, "file_size": file_size, "download_url": f"{Config.BASE_URL}/dl/{message_id}", "is_locked": is_locked}
    except: return JSONResponse({"error": "Server Error"}, status_code=500)
...
async def stream_file(message_id: int, request: Request, password: str = None):
    try:
        ...
        if hasattr(media, "file_unique_id"):
            file_data = await find_file_in_dbs(media.file_unique_id)
            if file_data:
                if file_data.get("password") and password != file_data['password']: raise HTTPException(status_code=401, detail="Password Required")
                if file_data.get("file_name"): file_name = file_data["file_name"]
        file_size = getattr(media, "file_size", 0)
        ...
        headers = {"Content-Disposition": f'attachment; filename="{file_name}"', "Accept-Ranges": "bytes", "Content-Length": str(file_size - start)}
        return StreamingResponse(streamer, status_code=206 if range_header else 200, headers=headers, media_type=mime_type)
