...
from bot.clone import db, cdb # Import both
...
async def get_dashboard(user_id: int):
    ...
    # Helper to get stats from a specific DB
    async def get_stats(database):
        pipeline = [{"$match": {"user_id": user_id}}, {"$group": {"_id": None, "totalSize": {"$sum": "$file_size"}, "count": {"$sum": 1}}}]
        res = await database.files.aggregate(pipeline).to_list(1)
        if res: return res[0]['totalSize'], res[0]['count']
        return 0, 0

    s1, c1 = await get_stats(db)
    s2, c2 = await get_stats(cdb)
    
    user_stats = {"used_storage": s1 + s2, "total_files": c1 + c2}

    # Get Files from BOTH (Limit 50 total)
    files = []
    
    async def fetch_files(database):
        cursor = database.files.find({"user_id": user_id}).sort("_id", -1).limit(25)
    ...
    await fetch_files(db)
    await fetch_files(cdb)

    admin_data = None
    clone_data = None
    if role == "admin":
        admin_data = {"total_users": await db.users.count_documents({}), "total_clones": await db.clones.count_documents({}), "total_files": (await db.files.count_documents({}) + await cdb.files.count_documents({}))}
    if role == "clone_admin":
...
async def search_files(user_id: int, query: str):
    files = []
    async def search_db(database):
        cursor = database.files.find({"user_id": user_id, "file_name": {"$regex": query, "$options": "i"}}).limit(10)
        async for doc in cursor:
            files.append({"file_unique_id": doc.get("file_unique_id"), "file_name": doc.get("file_name", "Unknown"), "file_size": doc.get("file_size", 0), "link": f"{Config.BASE_URL}/dl/{doc.get('log_msg_id')}"})
...
async def rename_file(data: FileAction):
    # Try rename in both
    r1 = await db.files.update_one({"file_unique_id": data.file_id, "user_id": data.user_id}, {"$set": {"file_name": data.new_name}})
    r2 = await cdb.files.update_one({"file_unique_id": data.file_id, "user_id": data.user_id}, {"$set": {"file_name": data.new_name}})
    return {"success": (r1.modified_count + r2.modified_count) > 0}
...
async def delete_file(data: FileAction):
    ...
    target_db = db
    if not doc:
        doc = await cdb.files.find_one({"file_unique_id": data.file_id, "user_id": data.user_id})
        target_db = cdb
    
    if doc:
        try: await tg_bot.delete_messages(Config.LOG_CHANNEL_ID, doc['log_msg_id']); 
        except: pass
        await target_db.files.delete_one({"_id": doc['_id']})
        return {"success": True}
