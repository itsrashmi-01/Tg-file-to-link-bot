...
async def get_main_menu(client, user_id, first_name):
    ...
    buttons = InlineKeyboardMarkup([[InlineKeyboardButton("🚀 My Dashboard", web_app=WebAppInfo(url=dashboard_url))], [InlineKeyboardButton("📂 My Files", web_app=WebAppInfo(url=files_url)), InlineKeyboardButton("⚙️ Settings", callback_data="settings")], [clone_btn], [InlineKeyboardButton("❓ Help", callback_data="help"), InlineKeyboardButton("ℹ️ About", callback_data="about")]])
    return text, buttons
