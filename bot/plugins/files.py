...
async def process_batch(client, mg_id, chat_id, user_id):
    ...
    links = ""
    current_db = get_db(client) # Select DB
    for msg in messages:
        try:
            ...
            await save_file_to_db(current_db, msg, log_msg, media, file_name, file_size)
...
async def process_file(client, message):
    try:
        ...
        current_db = get_db(client) # Select DB
        await save_file_to_db(current_db, message, log_msg, media, file_name, file_size)
...
async def save_file_to_db(database, user_msg, log_msg, media, file_name, file_size):
    unique_id = getattr(media, "file_unique_id", None) or getattr(media, "file_id", None)
    await database.files.insert_one({"user_id": user_msg.from_user.id, "log_msg_id": log_msg.id, "file_name": file_name, "file_size": file_size, "file_unique_id": unique_id, "timestamp": time.time()})
